...
namespace AsyncRircGisService.Gis.Infrastructure
{
    public class GisSoapFormatter
    {
        ...
        public string GetSoapRequest(string messageGuid)
        {
            var header = GetXmlHeader(messageGuid);
            var body = GetXmlBody(Template, ValuesDictionary);
            return GetSoapText(header, body);
        }

        private XmlNode GetXmlHeader( string messageGuid )
        {
